        ...
        stage('Tests') {
            steps {
                catchError(buildResult: 'SUCCESS', stageResult: 'FAILURE') {
                    sh """docker run --name project_tests --network selenoid project_tests --mode=remote --stand=${STAND} --browser_name=${BROWSER} --hub=${EXECUTOR_HUB} --hub_port=${EXECUTOR_PORT} -n ${STREAM_NUM}"""
                }
            }
        }
        stage('Copy Artefact') {
